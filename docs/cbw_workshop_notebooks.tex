...
\item
  correction\_method - set to fdr. by default g:Profiler uses g:Scs
\item
  organism - set to the custom\_gmt ID ( for this run it is - gp\_\_15KB\_4Xzp\_hzo) that we received when we uploaded our genetset file.
\end{itemize}
...
If you are running this notebook using the \href{https://hub.docker.com/r/risserlin/workshop_base_image}{baderlab workshop docker image} then the image comes pre-installed with the gsea jar that you can use to run gsea directly in the docker.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#path to GSEA jar }
\CommentTok{\# In order to run GSEA automatically you need to speciry the path to the gsea jar file.}
\CommentTok{\# the gsea\_jar aneeds to be the full path to the GSEA 4.3.2 directory that you}
\CommentTok{\# downloaded from GSEA. for example (/Users/johnsmith/GSEA\_4.3.2/gsea{-}cli.sh)}
\NormalTok{gsea\_jar }\OtherTok{\textless{}{-}}\NormalTok{ params}\SpecialCharTok{$}\NormalTok{gsea\_jar}

\CommentTok{\#navigate to the directory where you put the downloaded protocol files.}
...
\CommentTok{\#specify if you are using docker.}
\NormalTok{is\_docker }\OtherTok{\textless{}{-}}\NormalTok{ params}\SpecialCharTok{$}\NormalTok{is\_docker}
\end{Highlighting}
\end{Shaded}
